...
uct_gdaki_wqe_prepare_put_or_atomic(struct doca_gpu_dev_verbs_qp *qp,
                                    ...
                                    const uint64_t add)
{
    uint64_t *dseg = (uint64_t *)wqe_ptr + 4 + 2 * is_atomic;
    int ds = 3 + is_atomic;
    ...
    doca_gpu_dev_verbs_store_wqe_seg(dseg, (uint64_t *)&(dseg0));
}

__device__ static inline uint64_t
uct_gdaki_reserv_wqe_thread(struct doca_gpu_dev_verbs_qp *qp, unsigned count)
{
    cuda::atomic_ref<uint64_t, cuda::thread_scope_device> ref(qp->sq_rsvd_index);
    uint64_t wqe_base = ref.fetch_add(count, cuda::std::memory_order_acquire);

    while (wqe_base + count > qp->sq_wqe_pi + qp->sq_wqe_num) {
        uint64_t wqe_next = wqe_base + count;
        if (ref.compare_exchange_strong(wqe_next, wqe_base)) {
            return -1ULL;
        }
    }
...
{
    if (lane_id == 0) {
        wqe_base = uct_gdaki_reserv_wqe_thread(qp, count);
    }
